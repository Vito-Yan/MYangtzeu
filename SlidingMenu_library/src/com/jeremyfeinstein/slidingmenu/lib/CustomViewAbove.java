...
public class CustomViewAbove extends ViewGroup {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEventCompat.ACTION_MASK) {
			 ...
			 */
                ...
                // mLastMotionX 记录x的起点坐标
                int index = MotionEventCompat.getActionIndex(ev);
                //记录多点触碰的activity的第一点的id，初级多点触控拖动稳定性能
                mActivePointerId = MotionEventCompat.getPointerId(ev, index);
                mLastMotionX = mInitialMotionX = ev.getX();
                break;
            case MotionEvent.ACTION_MOVE:
                Log.e(TAG,"移动中...");
                if (!mIsBeingDragged) {  //如果
                    determineDrag(ev);
                    if (mIsUnableToDrag)
                        return false;
                }
                if (mIsBeingDragged) {   //正在拖拽
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        mViewBehind.drawShadow(mContent, canvas);                    //绘制阴影
        mViewBehind.drawFade(mContent, canvas, getPercentOpen());    //绘制淡入淡出
